...
import { addWatchReaction, addRenderReaction, clearReactions } from '../shared/utils';
import './Widgets.scss';
...
export class Widgets extends HTMLElement {
	...
	render() {
		...
		if (stores.widgetsStore.hasError) {
			return html` <app-error-card .text=${stores.widgetsStore.error.message}></app-error-card> `;
		}
	...
	get selectOptions() {
		...
		return stores.widgetsStore.widgets.map(
			(w) => html` <option .value=${w.id} ?selected=${w.id === id}>${w.name}</option> `
		);
	}
	...
	get widgetElement() {
		...
		return (
			widgetEl ??
			html`
				<app-error-card .title=${t('error')} .text=${t('widgets.error.notfound')}></app-error-card>
			`
		);
	}
