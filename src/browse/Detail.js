...
export class Detail extends HTMLElement {
	...
	render() {
		...
		if (stores.browseStore.hasError) {
			return html` <app-error-card .text=${stores.browseStore.error.message}></app-error-card> `;
		}

		if (stores.browseStore.selectedItem) {
			return html` <app-detail-card .data="${stores.browseStore.selectedItem}"></app-detail-card> `;
		}
